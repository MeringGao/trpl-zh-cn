...
* DONE [[file:ch15-00-smart-pointers.md][智能指针]] [6/6]
CLOSED: [2024-12-29 Sun 20:17]
:LOGBOOK:
- State "DONE"       from "TODO"       [2024-12-29 Sun 20:17]
:END:

** DONE [[file:ch15-01-box.md][使用 `Box<T>` 指向堆上数据]]
CLOSED: [2024-12-29 Sun 20:02]
:LOGBOOK:
- State "DONE"       from "TODO"       [2024-12-29 Sun 20:02]
:END:

Box<dyn Trait> 用于动态分发，适合在编译时无法确定具体类型的场景。它允许你在运行
时选择实现了某个 Trait 的不同对象，通常用于实现多态。使用 Box<dyn Trait> 会引入
一些运行时开销，因为它需要通过虚函数表（vtable）进行方法调用。

Box<impl Trait> 是一种返回类型的语法糖，用于静态分发。
当函数返回一个实现了某个 Trait 的具体类型时，可以使用 impl Trait 来隐藏具体类型。
这通常用于函数返回值的场景，省去了定义具体类型的必要性，而编译器会帮我们推断。

** DONE [[file:ch15-02-deref.md][使用 `Deref` Trait 将智能指针当作常规引用处理]]
CLOSED: [2024-12-29 Sun 20:17]
:LOGBOOK:
- State "DONE"       from "TODO"       [2024-12-29 Sun 20:17]
:END:
...
deref 方法返回值的引用，以及 *(y.deref()) 括号外边的普通解引用仍为必须的原因在于
所有权。如果 deref 方法直接返回值而不是值的引用，其值（的所有权）将被移出 self。
...
** DONE [[file:ch15-03-drop.md][使用 `Drop` Trait 运行清理代码]]
CLOSED: [2024-12-29 Sun 20:17]
:LOGBOOK:
- State "DONE"       from "TODO"       [2024-12-29 Sun 20:17]
:END:

对于智能指针模式来说第二个重要的 trait 是 Drop，其允许我们在值要离开作用域时执行
一些代码。可以为任何类型提供 Drop trait 的实现，同时所指定的代码被用于释放类似于
文件或网络连接的资源。

** DONE [[file:ch15-04-rc.md][`Rc<T>` 引用计数智能指针]]
CLOSED: [2024-12-29 Sun 20:16]
:LOGBOOK:
- State "DONE"       from "TODO"       [2024-12-29 Sun 20:16]
:END:

** DONE [[file:ch15-05-interior-mutability.md][`RefCell<T>` 与内部可变性模式]]
CLOSED: [2024-12-29 Sun 20:17]
:LOGBOOK:
- State "DONE"       from "TODO"       [2024-12-29 Sun 20:17]
:END:

** DONE [[file:ch15-06-reference-cycles.md][引用循环会导致内存泄漏]]
CLOSED: [2024-12-29 Sun 20:17]
:LOGBOOK:
- State "DONE"       from "TODO"       [2024-12-29 Sun 20:17]
:END:
...
* DONE [[file:ch16-00-concurrency.md][无畏并发]] [4/4]
CLOSED: [2024-12-29 Sun 20:43]
:LOGBOOK:
- State "DONE"       from "TODO"       [2024-12-29 Sun 20:43]
:END:

** DONE [[file:ch16-01-threads.md][使用线程同时地运行代码]]
CLOSED: [2024-12-29 Sun 20:19]
:LOGBOOK:
- State "DONE"       from "TODO"       [2024-12-29 Sun 20:19]
:END:

** DONE [[file:ch16-02-message-passing.md][使用消息传递在线程间通信]]
CLOSED: [2024-12-29 Sun 20:40]
:LOGBOOK:
- State "DONE"       from "TODO"       [2024-12-29 Sun 20:40]
:END:

** DONE [[file:ch16-03-shared-state.md][共享状态并发]]
CLOSED: [2024-12-29 Sun 20:41]
:LOGBOOK:
- State "DONE"       from "TODO"       [2024-12-29 Sun 20:41]
:END:

** DONE [[file:ch16-04-extensible-concurrency-sync-and-send.md][使用 `Sync` 与 `Send` Traits 的可扩展并发]]
CLOSED: [2024-12-29 Sun 20:43]
:LOGBOOK:
- State "DONE"       from "TODO"       [2024-12-29 Sun 20:43]
:END:

通过 Send 允许在线程间转移所有权几乎所有的 Rust 类型都是Send 的，不过有一些例外，
包括 Rc<T>：这是不能 Send 的，因为如果克隆了 Rc<T> 的值并尝试将克隆的所有权转移
到另一个线程，这两个线程都可能同时更新引用计数。

Sync 标记 trait 表明一个实现了 Sync 的类型可以安全的在多个线程中拥有其值的引用。
换一种方式来说，对于任意类型 T，如果 &T（T 的不可变引用）是 Send 的话 T 就是
Sync 的，这意味着其引用就可以安全的发送到另一个线程。类似于 Send 的情况，基本类
型是 Sync 的，完全由 Sync 的类型组成的类型也是 Sync 的。
...
* DONE [[file:ch18-00-oop.md][Rust 的面向对象编程特性]] [3/3]
CLOSED: [2024-12-29 Sun 20:52]
:LOGBOOK:
- State "DONE"       from "TODO"       [2024-12-29 Sun 20:52]
:END:

** DONE [[file:ch18-01-what-is-oo.md][面向对象语言的特点]]
CLOSED: [2024-12-29 Sun 20:50]
:LOGBOOK:
- State "DONE"       from "TODO"       [2024-12-29 Sun 20:50]
:END:

** DONE [[file:ch18-02-trait-objects.md][顾及不同类型值的 trait 对象]]
CLOSED: [2024-12-29 Sun 20:52]
:LOGBOOK:
- State "DONE"       from "TODO"       [2024-12-29 Sun 20:52]
:END:

** DONE [[file:ch18-03-oo-design-patterns.md][面向对象设计模式的实现]]
CLOSED: [2024-12-29 Sun 20:52]
:LOGBOOK:
- State "DONE"       from "TODO"       [2024-12-29 Sun 20:52]
:END:

* DONE [[file:ch19-00-patterns.md][模式与模式匹配]] [3/3]
CLOSED: [2024-12-30 Mon 13:02]
:LOGBOOK:
- State "DONE"       from "TODO"       [2024-12-30 Mon 13:02]
:END:
模式由如下一些内容组合而成：字面值, 解构的数组、枚举、结构体或者元组, 变量, 通配
符, 占位符

一些模式的例子包括x,(a, 3) 和 Some(Color::Red)。在模式为有效的上下文中，这些部分
描述了数据的形状。接着可以用其匹配值来决定程序是否拥有正确的数据来运行特定部分的
代码。

** DONE [[file:ch19-01-all-the-places-for-patterns.md][所有可能会用到模式的位置]]
CLOSED: [2024-12-30 Mon 12:56]
:LOGBOOK:
- State "DONE"       from "TODO"       [2024-12-30 Mon 12:56]
:END:

match ,if let ,while let, for 循环, let 语句, 函数参数
...
** DONE [[file:ch19-02-refutability.md][Refutability（可反驳性）: 模式是否会匹配失效]]
CLOSED: [2024-12-30 Mon 12:56]
:LOGBOOK:
- State "DONE"       from "TODO"       [2024-12-30 Mon 12:56]
:END:

能匹配任何传递的可能值的模式被称为是 不可反驳的（irrefutable）。一个例子就是 let
x = 5; 语句中的 x，因为 x 可以匹配任何值所以不可能会失败

对某些可能的值进行匹配会失败的模式被称为是 可反驳的（refutable）。一个这样的例子
便是 if let Some(x) = a_value 表达式中的 Some(x)；如果变量 a_value 中的值是 None
而不是 Some，那么 Some(x) 模式不能匹配。

函数参数、let 语句和 for 循环只能接受不可反驳的模式，因为当值不匹配时，程序无法
进行有意义的操作

if let 和 while let 表达式可以接受可反驳和不可反驳的模式，但编译器会对不可反驳的
模式发出警告，因为根据定义它们旨在处理可能的失败：条件表达式的功能在于它能够根据
成功或失败来执行不同的操作
...
** DONE [[file:ch19-03-pattern-syntax.md][模式语法]]
CLOSED: [2024-12-30 Mon 13:02]
:LOGBOOK:
- State "DONE"       from "TODO"       [2024-12-30 Mon 13:02]
:END:
...
* DONE [[file:ch20-00-advanced-features.md][高级特征]] [5/5]
CLOSED: [2024-12-30 Mon 22:34]
:LOGBOOK:
- State "DONE"       from "TODO"       [2024-12-30 Mon 22:34]
:END:

** DONE [[file:ch20-01-unsafe-rust.md][不安全的 Rust]]
CLOSED: [2024-12-30 Mon 22:26]
:LOGBOOK:
- State "DONE"       from "TODO"       [2024-12-30 Mon 22:26]
:END:
...
回到第四章的 “悬垂引用” 部分，那里提到了编译器会确保引用总是有效的。不安全 Rust
有两个被称为 裸指针（raw pointers）的类似于引用的新类型。和引用一样，裸指针是不
可变或可变的，分别写作 *const T 和 *mut T。这里的星号不是解引用运算符；它是类型
名称的一部分。在裸指针的上下文中，不可变 意味着指针解引用之后不能直接赋值
...
#+begin_src rust
  ...
  // 不过不能 解引用 裸指针和读取其指向的数据。现在我们要做的就是对裸指针使用解引用运算符 *，这需要一个 unsafe 块
  unsafe {
          println!("r1 is: {}", *r1);
          println!("r2 is: {}", *r2);
      }
...
** DONE [[file:ch20-03-advanced-traits.md][高级 trait]]
CLOSED: [2024-12-30 Mon 22:26]
:LOGBOOK:
- State "DONE"       from "TODO"       [2024-12-30 Mon 22:26]
:END:

实现了多个 trait 且方法同名如何指定调用某个 triat 的方法
#+begin_src rust
  fn main() {
      let person = Human;
      Pilot::fly(&person);
...
newtype 模式用以在外部类型上实现外部 trait,从而绕开孤儿规则
#+begin_src rust
  use std::fmt;
...
** DONE [[file:ch20-04-advanced-types.md][高级类型]]
CLOSED: [2024-12-30 Mon 22:33]
:LOGBOOK:
- State "DONE"       from "TODO"       [2024-12-30 Mon 22:33]
:END:
...
** DONE [[file:ch20-05-advanced-functions-and-closures.md][高级函数与闭包]]
CLOSED: [2024-12-30 Mon 22:33]
:LOGBOOK:
- State "DONE"       from "TODO"       [2024-12-30 Mon 22:33]
:END:

函数指针, 函数满足类型 fn（小写的 f），不要与闭包 trait 的 Fn 相混淆。fn 被称为
...
返回闭包: 闭包表现为 trait，这意味着不能直接返回闭包。对于大部分需要返回 trait
的情况，可以使用实现了期望返回的 trait 的具体类型来替代函数的返回值。但是这不能
用于闭包，因为它们没有一个可返回的具体类型；例如不允许使用函数指针 fn 作为返回值
类型。
...
** DONE [[file:ch20-06-macros.md][宏]]
CLOSED: [2024-12-30 Mon 22:34]
:LOGBOOK:
- State "DONE"       from              [2024-12-30 Mon 22:34]
:END:

宏小册

* DONE [[file:ch21-00-final-project-a-web-server.md][最后的项目：构建多线程 web server]] [3/3]
CLOSED: [2024-12-30 Mon 22:39]
:LOGBOOK:
- State "DONE"       from "TODO"       [2024-12-30 Mon 22:39]
:END:

** DONE [[file:ch21-01-single-threaded.md][建立单线程 web server]]
CLOSED: [2024-12-30 Mon 22:39]
:LOGBOOK:
- State "DONE"       from "TODO"       [2024-12-30 Mon 22:39]
:END:

** DONE [[file:ch21-02-multithreaded.md][将单线程 server 变为多线程 server]]
CLOSED: [2024-12-30 Mon 22:39]
:LOGBOOK:
- State "DONE"       from "TODO"       [2024-12-30 Mon 22:39]
:END:

** DONE [[file:ch21-03-graceful-shutdown-and-cleanup.md][优雅停机与清理]]
CLOSED: [2024-12-30 Mon 22:39]
:LOGBOOK:
- State "DONE"       from "TODO"       [2024-12-30 Mon 22:39]
:END:
